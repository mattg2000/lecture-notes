...
- **UUID** - Universally Unique IDentifier.  The unique value that identifies an ID
  ...
  - We use version 4 UUIDs, which are randomly created
    - chance of collision between two UUIDs is vanishingly small: 1 in 2^122, or approximately 1 in  5,300,000,000,000,000,000,000,000,000,000,000,000 
- Our UUIDs need to be modified to fit into the DB
  - remove dashes
  - `HEX()` and `UNHEX()` - functions that translate to and from hexidecimal numbers
  - All UUIDs are in hexidecimal, and should be converted to characters using `UNHEX()` before being stored in the DB.
  - UUIDs retrieved from the database will have to be translated back into hexidecimal using `HEX()` to make them human readable.
  - https://www.w3resource.com/mysql/string-fUNunctions/mysql-unhex-function.php

## INSERT
- inserts data into the databases
- `INSERT INTO row (column1, column,2, column3, column4) VALUES ("value1", "value2", "value3", "value4");`
- `INSERT INTO profile(profileId, profileActivationToken, profileAtHandle, profileEmail, profileHash, profilePhone) VALUES (UNHEX("4d7342ff55c0483097a0da7b2b6a9d9d"), "nananananananananananananananana", "myAtHandle", "email@email.com", "nanananananananananananananananananananananananananananananananananananananananananananananananaa", "505-866-5309");`

### Inserting into tables with Foreign keys.
- `INSERT INTO tweet(tweetId, tweetProfileId, tweetContent, tweetDate) VALUES (UNHEX("2435f00e555d455d92fdf34f1dd255e2"), UNHEX("4d7342ff55c0483097a0da7b2b6a9d9d"), "I hate dates because I never get them", NOW());`
...
- `UPDATE tweet SET tweetProfileId = UNHEX("4d7342ff55c0483097a0da7b2b6a9d9d"), tweetContent="I get dates now" tweetDate = NOW() WHERE tweetId = UNHEX("2435f00e555d455d92fdf34f1dd255e2");`
